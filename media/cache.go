...
func MediaCache() *LocalMediaCache {
	if _defaultMediaCache == nil {
		...
		if !disable {
			...
			log.WithField("root", rootVal).Info("mediacache: initialized")
		}
	}
...
func (c *LocalMediaCache) Store(key string, data []byte) error {
	...
	if err != nil {
		log.WithFields(map[string]interface{}{"filename": filename, "error": err}).Error("mediacache: failed to write file")
		return err
	}
	log.WithFields(map[string]interface{}{"filename": filename, "datasize": len(data)}).Info("mediacache: stored")
	return nil
}
...
func (c *LocalMediaCache) Get(key string) ([]byte, error) {
	...
	if err != nil {
		log.WithFields(map[string]interface{}{"filename": filename, "error": err}).Error("mediacache: failed to read file")
		return nil, err
	}
	return data, nil
}
